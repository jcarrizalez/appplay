...
export default class Container2 extends Component {
    ...
    render(){
        return(
            <Area>
                <AreaTop onBack={() => this.props.navigation?.goBack()}/>
                <AreaVideo />
                <Cast />
                <TouchableOpacity style={{borderRadius: 20,width: 40, height: 40, alignItems:'center', justifyContent: "center"  }} onPress={() => {
                    console.log('onPress')
                    //GoogleCastV3.loadVideo('https://www.latatv.com:444/live/5dcc76a163dbe/index.m3u8');
                }}>
...
class Cast extends Component {
    ...
    componentDidMount() {
        ...
        GoogleCastV3.addCastStateListener(state => {
            console.log(state)
          if(state===4){
            this.setState({googleCast:true})
          }
          else if(state===2){
            this.setState({googleCast:false})
          }
        })
        GoogleCastV3.addCastMessageListener(message => {
          console.log('addCastMessageListener',message)
    ...
    render(){
        //console.log(GoogleCastV3)
        //<CastButton color="#fff" ref={c => this.myCastButton = c } />
            
        return(
            <TouchableOpacity onPress={()=> this.prueba()} style={{backgroundColor:'green', marginRight:5, width:40, height:40, borderRadius:20, paddingTop: 20}}>
                <View color="#fff" ref={c => this.myCastButton = c } />
            </TouchableOpacity>
...
const Barra = ({children}) =>
    <View style={{backgroundColor:'blue', width:'100%', height:'10%', flexDirection: 'row', alignItems: 'center', justifyContent: "space-between"}}>
        {children}
    </View>;

const AreaTop = ({onBack}) =>
    <Barra>
        {/*BACK*/}
        <TouchableOpacity onPress={()=> onBack()} style={{backgroundColor:'red', marginLeft:5, width:40, height:40, borderRadius:20, padding: 9}}>
            <Image source={ require( './img/back.png' ) } />
        </TouchableOpacity>
        <Text>ViewTop</Text>
        {/*Fullscreen*/}
        <TouchableOpacity onPress={()=> null} style={{backgroundColor:'red', marginRight:5, width:40, height:40, borderRadius:20, padding: 7}}>
            <Image source={ (false)? require( './img/shrink.png' ) : require( './img/expand.png' ) } />
        </TouchableOpacity>
    </Barra>;



const AreaButtom = ({myCastButton}) =>
    <Barra>
        <Text>!!</Text>

        <Text>ViewButtom</Text>
        {/*GoogleCast*/}
        <TouchableOpacity onPress={()=> myCastButton.click()} style={{backgroundColor:'red', marginRight:5, width:40, height:40, borderRadius:20, paddingTop: 20}}>
            <CastButton color="#fff" ref={c => myCastButton = c } />
        </TouchableOpacity>

    </Barra>;

const AreaVideo = () =>
    <View style={{backgroundColor:'green', width:'100%', height:'80%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
        <Text>xxxxx</Text>
    </View>;


const ButtomBack = ({onBack}) =>
    <TouchableOpacity
        underlayColor="transparent"
        activeOpacity={ 0.3 }
        onPress={()=> onBack()}
        style={{...styles.controls.control, backgroundColor:'red',}}
    >
        <Image source={ require( './img/back.png' ) } style={ styles.controls.back } />
    </TouchableOpacity>
...
export class Container extends Component {
    ...
    */
    ...
    setVolumePosition( position = 0 ) {
        ...
        //SystemSetting.setVolume((position/this.player.volumeWidth));
        this.setState( state );
    }
     ...
     */
    calculateVolumePositionFromVolume(volume=null, volumeWidth=null) {
        if(volume===null && volumeWidth===null){
            return volumeWidth * volume;
        }
    ...
    */
     ...
     */
    setVolumeNat(cant=0){
        let strToNum = new Number(cant);
        /*
        SystemSetting.getVolume().then((volume)=>{

            this.setVolumePosition( ((volume*this.player.volumeWidth)+strToNum) );
            const position = this.calculateVolumePositionFromVolume(volume, volumeWidth);
            this.mounted = true;
    ...
    componentDidMount() {
        /*
        GoogleCastV3.addCastStateListener(state => {
          if(state===4 && this.state.googleCast!==true){
            this.setState({googleCast:true})
          }
          else if(state===2 && this.state.googleCast===true){
            this.setState({googleCast:false})
          }
        })
        GoogleCastV3.addCastMessageListener(message => {
    ...
    */
    ...
    renderVolume() {
        ...
        return (
            <View style={ styles.volume.container }>
                ...
                >
                <Image style={ styles.volume.icon } source={ require( './img/volume.png' ) } />
                </TouchableHighlight>
    ...
    renderFullscreen() {
        ...
        let source = this.state.isFullscreen === true ? require( './img/shrink.png' ) : require( './img/expand.png' );
        return this.renderControl(
            <Image source={ source } />,
            this.methods.toggleFullscreen,
    ...
    renderPlayPause() {
        ...
        let source = this.state.paused === true ? require( './img/play.png' ) : require( './img/pause.png' );
        return this.renderControl(
            <Image source={ source } />,
    ...
    renderPlayGoogleCast(){
        if(this.state.googleCast!==true) return null
    ...
    renderGoogleCast() { 
        return(
            <View style={{ width: 80, height: 40,  flexDirection: 'row', alignItems:'center', justifyContent: "center" }}>
                
                {this.renderPlayGoogleCast()}

                <View style={{width: 40, height: 40, alignItems:'center', justifyContent: "center",  }} >
                
                    <TouchableOpacity style={{borderRadius: 20,width: 40, height: 40 }} onPress={() => {
                      this.myCastButton.click()
                      //GoogleCastV3.send('TESTJUAN')
                    }} />
                    <CastButton style={{position:'absolute'}} color="#fff" ref={c => this.myCastButton = c } />
                </View>
        ...
        return (
            ...
            >
                <View style={[ styles.player.container, this.styles.containerStyle, {
                    ...
                    //width: Dimensions.get('window')[this.state.isFullscreen? 'height' : 'width'] ,
                    //height: Dimensions.get('window')[this.state.isFullscreen? 'width' : 'height'] ,
                    //transform: [{ rotate: this.state.isFullscreen? '90deg' : '0deg'}],
                } ]}>
                    <Video
                        { ...this.props }
                        key={1234}
                        ...
                        //resizeMode={ !this.state.isFullscreen? 'cover' : 'stretch'  }
                        ...
                        */
                        ...
                        style={[/* styles.player.video,*/ this.styles.videoStyle,
                            {
                                ...
                                position: 'absolute',
                                //width: Dimensions.get('window')[this.state.isFullscreen? 'height' : 'width'] ,
                                //height: Dimensions.get('window')[this.state.isFullscreen? 'height' : 'width'] ,
                                top: 0,
                                right: 0,
                                bottom: 0,
                                left: 0,
                        ...
                        //source={{ uri: 'http://192.168.19.223/tets.mp4' }} 
                        //source={{ uri: 'http://192.168.0.137/tets.mp4' }} 
                    />
                    { this.renderTopControls() }
                    <RenderError error={this.state.error} />
                    <RenderLoader loading={this.state.loading}  animations={this.animations}/>
...
const RenderLoader = ({loading, animations}) => (!loading)? null : 
...
const RenderError = ({error}) => (!error)? null : 
    <View style={ styles.error.container }>
        <Image source={ require( './img/error-icon.png' ) } style={ styles.error.icon } />
